...
export class RomeComponent implements AfterViewInit {
  ...
  @ViewChildren('autoVid') autoPlayVideos!: QueryList<ElementRef<HTMLVideoElement>>;
  ...
  ngAfterViewInit(): void {
    // Attempt immediate play on all videos
    setTimeout(() => {
      this.autoPlayVideos?.forEach((vidRef) => {
        const el = vidRef.nativeElement;
        el.muted = true;
        el.setAttribute('playsinline', 'true');
        el.play().catch(() => {
          // Fallback: observe visibility and play when in view
        });
      });
    });

    // Intersection observer to ensure autoplay when visible
    ...
    this.autoPlayVideos?.forEach((vidRef) => observer.observe(vidRef.nativeElement));
  }
